 ...
 * {@link NeuralHardwareConfig#DEFAULT_HIT_THE_ZONE_INJECTION_VOLTAGE} stimulus to the neural hardware via its
...
public class HitTheZoneHardwareAI
    ...
    private final double injectionVoltage;
    ...
    private void updateInjectionForZoneTransition(HitTheZoneState state) {
        if (!wasInZone && state.inZone()) {
            debug(String.format("zone enter -> inject %.3fV", injectionVoltage));
            injector.injectVoltage(injectionVoltage);
    ...
    public HitTheZoneHardwareAI(String name,
        ...
        this(
                ...
                signalSource,
                injector,
                NeuralHardwareConfig.DEFAULT_HIT_THE_ZONE_INJECTION_VOLTAGE,
