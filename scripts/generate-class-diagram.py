...
def base_lines(title: str) -> list[str]:
    return [
        "@startuml",
        f"title {title}",
...
def ordered_summary_tiers(top_level_packages: list[str]) -> list[list[str]]:
    remaining = set(top_level_packages)
    ordered: list[list[str]] = []

    for tier in SUMMARY_TIERS:
        active = [package_name for package_name in tier if package_name in remaining]
        if active:
...
def write_summary_diagram(
    ...
    lines = base_lines("Package Summary Diagram")
    ...
    for upper_tier, lower_tier in zip(tiers, tiers[1:]):
        for upper_name in upper_tier:
            for lower_name in lower_tier:
                lines.append(
                    f"{package_nodes[(upper_name, '')].alias} -[hidden]down-> {package_nodes[(lower_name, '')].alias}"
                )
        lines.append("")
    ...
    for source, target in sorted(usage_relations):
        ...
        if source_type.top_level_package != target_type.top_level_package:
            top_level_package_relations.add((source_type.top_level_package, target_type.top_level_package))

    for source_top_level, target_top_level in sorted(top_level_package_relations):
        lines.append(
            f"{package_nodes[(source_top_level, '')].alias} ..> {package_nodes[(target_top_level, '')].alias}"
        )
